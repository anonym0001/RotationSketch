...
#ifdef _WIN64
inline uint64_t __builtin_bswap64(const uint64_t &x)
{
	uint64_t y = 0;
	char* src = (char*)&x;
	char* dst = ((char*)&y) + 7;
...
inline uint32_t __builtin_bswap32(const uint32_t &x)
{
	uint32_t y = 0;
	char* src = (char*)&x;
	char* dst = ((char*)&y) + 3;
...
RACE::RACE(size_t R, size_t range){
	...
	// _R=med*mean, first calculate median_num means of mean_num counters, then calculate the median of median_num means
	size_t medians = (size_t)floor(sqrt(_R));
	while (_R%medians != 0)
	{
		medians--;
	}
...
{
	//double* means = new double[median_num]();
	//for (size_t r = 0; r < _R; r++)
	//{
	//	size_t index = hashes[r] % _range;
	//	means[r/mean_num] += _sketch[r*_range + index]; 
	//}
	//std::sort(means, means + median_num);
	//double res;
	//if (median_num & 1)
	//{
	//	res =  means[median_num / 2] / mean_num;
	//}
	//else
	//{
	//	res =  (means[median_num / 2] + means[median_num / 2 + 1]) / 2 / mean_num;
	//}
	//delete[] means;
	//return res;
...
*/
	...
	for (size_t r = 0; r < _R; r++){
		...
		race_sketch_t difference = (race_sketch_t)(dataset_size - sum);
		for(size_t i = 0; i < _range; i++){
